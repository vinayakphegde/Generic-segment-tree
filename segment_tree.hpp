...
template<typename T>
class segment_tree<T, operation::concat<T> >
{
	private:
	mutable vector<T> tree;
	mutable vector<T> lazy;
	...
	void build_tree(iter &begin_, iter &end_, int start,int end, int np)
	{
		...
		tree[np]=operation::concat<T>()(tree[2*np+1],tree[2*np+2]);
	}
	
	void propagate(int start, int end, int np) const
	{
		if (lazy[np].compare("")!=0)
    	{
    		// cout << "hello";
			if(update_assign[np] == true)
			{
	...
	T query_util(int start,int end,int query_start,int query_end,int np) 
	{
		...
		if(start>query_end or end<query_start)
		{
			return operation::concat<T>().neutral_element;
		}
		if(query_start<=start and query_end>=end)
		{
			return tree[np];
		...
		int mid=mid(start,end);
		string left=query_util(start,mid,query_start,query_end,2*np+1);
		string right=query_util(mid+1,end,query_start,query_end,2*np+2);
		return operation::concat<T>()(left,right);
	}
	
	void update_util(int start,int end,int index,string value,int np)
	{
		if(index<start or index>end)
			return;
	...
	void updateRange_util(int start,int end,int query_start,int query_end,string value,int np)
	{
		propagate(start, end, np);
	...
	void assignRange_util(int start,int end,int query_start,int query_end,string value,int np)
	{
		propagate(start, end, np);
    ...
    {
        private:
        const segment_tree<T,operation::concat<T> > *tree_;
	    int index_;

        public:
        iterator(const segment_tree<T, operation::concat<T> > &tree, int index):tree_(&tree), index_(index)
        { }
	...
	// Copy Constructor
	segment_tree(const segment_tree<T,operation::concat<T> >& other): size_(other.size_), n(other.n)
	{
		tree = other.tree;
		lazy = other.lazy;
		update_assign = other.update_assign;
	...
	void updateRange(int query_start,int query_end,string value){
		if(query_start < 0 || query_end > size_ )
			return;
		return updateRange_util(0,size_-1,query_start,query_end,value,0);
	}

	void assignRange(int query_start,int query_end,string value){
		if(query_start < 0 || query_end > size_ )
			return;
		return assignRange_util(0,size_-1,query_start,query_end,value,0);
	}
